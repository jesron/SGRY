  ...
  <div class="side-menu-wrapper">
    ...
    <Menu ref="menu" v-show="!collapsed" :active-name="activeName" :open-names="openedNames" :accordion="accordion" :theme="theme" width="auto" @on-select="handleSelect">
      <template v-for="item in menuList">
        ...
        <!--  多个子节点 -->
        <template v-else>
          <side-menu-item :zsxqnum="zsxqnum"  v-if="showChildren(item)" :key="`menu-${item.name}`" :parent-item="item"></side-menu-item>
    ...
    <div class="menu-collapsed" v-show="collapsed" :list="menuList">
      <template v-for="item in menuList">
        <collapsed-menu  :zsxqnum="zsxqnum" v-if="item.children && item.children.length > 1" @on-click="handleSelect" hide-title :root-icon-size="rootIconSize" :icon-size="iconSize" :theme="theme" :parent-item="item" :key="`drop-menu-${item.name}`" ></collapsed-menu>
...
export default {
  ...
  mounted () {
    ...
    let o=this;
    // getCertificateListAllcount().then(res => {
    //   o.zsxqnum=res.data.data;
    // });
