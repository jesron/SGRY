...
export default {
  ...
  computed: {
    ...
    colNum () {
      return this.colnumref//this.pasteDataArr[0] ? this.pasteDataArr[0].length : 0
    }
  },
  methods: {
    handleKeyup (e) {
    ...
    checkColNumInEveryRow () {
      ...
      //if (len === 0) return
   //   console.log(this.colnumref);
      
      while (++i < len) {
        let item = this.pasteDataArr[i]
        if (item.length !== this.colnumref && (!(i === len - 1 && item.length === 1 && item[0] === '') || i !== len - 1)) {
